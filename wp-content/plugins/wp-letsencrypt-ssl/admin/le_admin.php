...
class WPLE_Admin
{
    ...
    public function wple_rateus()
    {
        ...
        if ( file_exists( $cert ) ) {
            if ( isset( $_GET['page'] ) && $_GET['page'] == 'wp_encryption' && !isset( $_GET['success'] ) ) {
                return;
            }
    ...
    public function wple_success_block( &$html )
    {
      ...
      <div class="wple-success-form">';
        ...
        <h3 style="width: 87%; margin: 0px auto; color: #7b8279; font-weight:400;">' . $this->wple_kses( __( 'You just completed major task of generating SSL certificate! Now we have ONE final step to complete.', 'wp-letsencrypt-ssl' ) ) . '</h3>';
            $html .= WPLE_Trait::wple_progress_bar();
            $html .= '   
        ...
        <ul class="download-ssl-certs">
          <li>
            <span>1. Download SSL certificates from below</span>
            <ul>
          ...
          <li>2. Open <strong>SSL/TLS</strong> option on your cPanel</li>
          <li>3. Copy/Paste cert files into appropriate fields</li>
          <li>4. Click on <strong>Install certificate</strong></li>
          <li>5. Please wait few minutes and click on <strong>Enable HTTPS Now</strong> button</li>
        </ul>
        </div>
        </div>  
            ...
            <div class="wple-success-cols">
              <div>
                ...
                <p>' . sprintf( esc_html__( 'Download the certs and send it to your hosting support to install it for you. If you have root SSH access, you can refer our %sDOCS%s for server specific instructions.', 'wp-letsencrypt-ssl' ), '<a href="https://wpencryption.com/docs/" target="_blank">', '</a>' ) . '</p>
              </div>
              <div>
              ...
              <div>
                <h3>' . esc_html__( "Don't have both cPanel & SSH?", 'wp-letsencrypt-ssl' ) . '</h3>
                <p>Why pay for an SSL certificate alone when you can get <strong>SSL</strong> + <strong>Security Firewall</strong> + <strong>CDN Performance</strong> with our <a href="https://wpencryption.com/#firewall" target="_blank">Secure Firewall</a> Service for even lower cost!.</p>
              </div>              
            </div>
    ...
    public function wple_domain_verification()
    {
        ...
        if ( FALSE !== $estage && $estage == 2 && !isset( $_GET['subdir'] ) && !isset( $_GET['error'] ) && !isset( $_GET['includewww'] ) && !isset( $_GET['wpleauto'] ) && isset( $_GET['page'] ) && $_GET['page'] == 'wp_encryption' && !isset( $_GET['success'] ) ) {
            wp_redirect( admin_url( '/admin.php?page=wp_encryption&subdir=1' ), 302 );
            exit;
        }
        
        
        if ( FALSE !== $estage && $estage == 5 && !isset( $_GET['subdir'] ) && !isset( $_GET['error'] ) && !isset( $_GET['includewww'] ) && !isset( $_GET['wpleauto'] ) && isset( $_GET['page'] ) && $_GET['page'] == 'wp_encryption' && !isset( $_GET['resume'] ) && !isset( $_GET['nossl'] ) ) {
  ...
  <?php 
    ...
    public function wple_completed_block( &$html )
    {
        ...
        $this->wple_restartnotice_after_renewal( $future, $html );
        $renewlink = '<a href="#" class="letsrenew wple-tooltip disabled" data-tippy="This renew button will get enabled during last 30 days of current SSL certificate expiry">Renew SSL Certificate</a>';
        if ( $daysleft <= 30 ) {
            $renewlink = '<a href="' . admin_url( '/admin.php?page=wp_encryption&restart=1' ) . '" class="letsrenew">Renew SSL Certificate</a>';
        }
        $headline = 'Kudos! WP Encryption just saved you several $$$ in SSL Certificate Fee.';
        $html .= '<div id="wple-completed">
        <div class="wple-completed-review">
          <h2>' . $headline . '</h2>
          <p>Can you please do us a BIG favor by leaving a <span class="dashicons dashicons-star-filled"></span><span class="dashicons dashicons-star-filled"></span><span class="dashicons dashicons-star-filled"></span><span class="dashicons dashicons-star-filled"></span><span class="dashicons dashicons-star-filled"></span> rating on WordPress.org</p>
          <a href="https://wordpress.org/support/plugin/wp-letsencrypt-ssl/reviews/#new-post" target="_blank" class="letsrate">LEAVE A RATING <span class="dashicons dashicons-external"></span></a>
          ' . $renewlink . '
          <small>Just takes few seconds</small>
        </div>';
        if ( file_exists( $cert ) && isset( $leopts['expiry'] ) ) {
          ...
          <div class="progress--circle progress--' . esc_attr( $daysleft ) . '">
            <div class="progress__number"><strong>' . esc_html( $daysleft ) . '</strong><br><small>Days</small></div>
          </div>  
    ...
    private function wple_restartnotice_after_renewal( $newcertdate, &$html )
    {
        ...
        SSL Certificate have been successfully renewed but your frontend site is showing older SSL certificate. Please restart your server via your hosting panel or SSH once for new SSL certificate to start showing on your site (<a href="https://wpencryption.com/docs/how-to-restart-server/" target="_blank">How to restart server?</a>).
        </div>';
        }
    }
